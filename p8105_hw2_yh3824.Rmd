...
```{r}
tidy_zillow = 
  left_join(zipcode_df, zori_df, by = "zip_code") |> 
  mutate(county = coalesce(county.x, county.y)) |> 
  select(county, -county.x, -county.y, neighborhood, zip_code, year, month, zori)
```
...
`r nrow(zipcode_only)` zip codes are in the zip code dataset, but not the zillow rental price dataset. The zip codes that are in `zipcode_df` but not `zori_df` are `r paste(zipcode_only$zip_code, collapse = ", ")`
